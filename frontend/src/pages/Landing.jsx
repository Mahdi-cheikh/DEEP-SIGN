...
const FOR_WHO = [
  { title: 'Deaf and hard-of-hearing communities', body: 'Lets you sign directly to anyone with a webcam — your gestures become spoken words instantly, with no special equipment needed.' },
  { title: 'Students and educators', body: 'A free, browser-based tool to practice common gestures and ASL signs. Detection feedback is instant and visual.' },
  { title: 'Developers and researchers', body: 'Open-source codebase, deterministic geometry classifier, and a clean React frontend. Fork it, extend the vocabulary, or train your own model.' },
  { title: 'Accessibility teams', body: 'Ship inclusive features without bloated SDKs or paid services. Runs entirely client-side — privacy by default.' },
];

const HOW_STEPS = [
  { n: '01', title: 'You sign in front of your camera', body: 'Browser-only — no install, no account. Webcam frames are processed locally and never uploaded.' },
  { n: '02', title: 'MediaPipe finds your hand',         body: 'Google’s WASM Hand Landmarker locates 21 joints on each hand at ~30 fps directly on your device.' },
  { n: '03', title: 'A classifier names the sign',       body: 'Geometric rules + MediaPipe’s gesture model identify what you are signing and how confident we are.' },
  { n: '04', title: 'Words become a sentence — and speech', body: 'Hold each sign for half a second to add it; the browser’s text-to-speech speaks the full sentence aloud.' },
];
      ...
      <section className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        <div>
          <span className="inline-flex items-center gap-2 badge bg-white/70 text-brand-700 border border-brand-100 mb-5">
            ...
            Real-time · In-browser · No account needed
          </span>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link to="/history" className="btn-secondary !px-5 !py-3 text-base">
              View history
            </Link>
          </div>
        ...
        <div className="relative">
          ...
          <style>{`@keyframes floaty { 0% { transform: translateY(0); } 100% { transform: translateY(-4px); } }`}</style>

          <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full opacity-50 blur-2xl -z-10"
               style={{ background: 'radial-gradient(closest-side, #4f90ff, transparent)' }} />
          <div className="absolute -bottom-8 -left-8 w-32 h-32 rounded-full opacity-50 blur-2xl -z-10"
          ...
          {FOR_WHO.map((f) => (
            <div key={f.title} className="card">
              <div className="flex items-start gap-3">
                <div className="shrink-0 w-10 h-10 rounded-xl grid place-items-center text-white"
      ...
      <section className="mt-24">
        <div className="card grid sm:grid-cols-[180px_1fr] sm:items-center gap-6 sm:gap-8 p-6 sm:p-8">
          <div className="mx-auto sm:mx-0">
            <div className="relative w-32 h-32 sm:w-40 sm:h-40 rounded-2xl overflow-hidden ring-4 ring-white shadow-lg"
              ...
              />
              <div data-initials style={{ display: 'none' }}
          ...
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-extrabold mt-3 text-slate-900">Mehdi Cheikh</h2>
            <p className="mt-3 text-slate-600 leading-relaxed">
              ...
              that lets anyone be heard the moment they sign — on any device, anywhere.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              <a href="https://github.com/Mahdi-cheikh" target="_blank" rel="noreferrer noopener"
